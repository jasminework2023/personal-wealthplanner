...
function CheckupCalc({ onSaveResult, onNavigate }) {
  ...
  return (
    <CalcLayout
      ...
      onSave={() => onSaveResult({ id: "checkup", title: "Check-Up Finansial", value: `${Math.round(score)}/100 (${grade})`, plan: { type: "cashflow", targetAmount: expenses * 12, income, expense: expenses, timeframe: 5, preparedAssets: assets } })}
...
export function FinancialCalculator({ onOpen }: { onOpen: (id: string) => void }) {
  return <div className="wp-calculator" style={{display:"grid",gap:20}}>
    <div><div style={{fontSize:11,fontWeight:700,letterSpacing:".1em",textTransform:"uppercase",color:"#71827b"}}>WEALTHPLANNER</div><h1 style={{fontSize:28,lineHeight:1.15,color:"#173b32",margin:"7px 0 6px"}}>Kalkulator Finansial</h1><p style={{margin:0,fontSize:13,color:"#667a72"}}>Hitung, simulasikan, lalu simpan rencana ke Proteksi Finansial tanpa membuka tab baru.</p></div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(230px,1fr))",gap:12}}>
      {CALCULATOR_META.map((c)=><button key={c.id} type="button" onClick={()=>onOpen(c.id)} style={{textAlign:"left",padding:18,border:"1px solid rgba(20,50,40,.10)",background:"white",borderRadius:16,cursor:"pointer"}}><div style={{fontSize:15,fontWeight:700,color:"#173b32"}}>{c.title}</div><div style={{fontSize:12,lineHeight:1.55,color:"#71827b",marginTop:7}}>{c.desc}</div><div style={{fontSize:12,fontWeight:700,color:"#0f5d46",marginTop:14}}>Buka kalkulator →</div></button>)}
    </div>
  </div>;
}
