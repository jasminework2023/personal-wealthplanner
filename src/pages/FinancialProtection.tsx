...
export function FinancialProtection() {
  ...
  const totalProtectionNeed = useMemo(() => plans.reduce((sum, p) => sum + Math.max((p.targetAmount || p.obligationAmount || 0) - (p.preparedAssets || 0), 0), 0), [plans]);
  ...
  return <div className="flex flex-col gap-6">
    ...
    <Card className="overflow-hidden border-forest-100 bg-gradient-to-br from-white via-forest-50/50 to-rose-50/30">
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-wrap gap-3"><a href="https://gri.my.id/f1/GJ5115" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg bg-forest-600 px-4 py-2.5 text-[13px] font-semibold text-white hover:bg-forest-700">Konsultasi dengan Life Planner <ArrowRight size={14}/></a></div>
      </div>
    </Card>
    ...
    <Card>
      ...
      <div className="mt-4 flex snap-x gap-4 overflow-x-auto pb-3 pr-1 [scrollbar-width:thin]">
        {protectionCards.map((card) => <article key={card.title} className="w-[min(84vw,360px)] shrink-0 snap-start rounded-2xl border border-charcoal/10 bg-white p-5 shadow-sm">
          ...
          <p className="mt-3 text-[12px] leading-5 text-charcoal/55">{card.body}</p>
          <p className="mt-4 text-[11px] font-semibold uppercase tracking-wide text-forest-700">Cocok untuk</p>
          <ul className="mt-2 space-y-1.5 text-[11px] leading-4 text-charcoal/60">{card.cocok.map((x) => <li key={x}>• {x}</li>)}</ul>
