...
export function FinancialProtection() {
  ...
  const totalNeed = useMemo(() => plans.reduce((sum, p) => sum + Math.max((p.targetAmount || p.obligationAmount || 0) - (p.preparedAssets || 0) - (p.existingProtection || 0), 0), 0), [plans]);
  const totalExisting = useMemo(() => plans.reduce((sum, p) => sum + (p.existingProtection || 0), 0), [plans]);

  function openAdd() { setAdding(true); setSelectedType(null); }
  ...
  function addPlan() {
    ...
    const saved = saveFinancialPlan({ calculatorId: `manual-${selectedType}-${Date.now()}`, type: (selectedType === "other" ? "cashflow" : selectedType) as any, name: label, targetAmount: target, preparedAssets: prepared, existingProtection: existing, timeframe });
    setPlans(loadFinancialPlans()); setAdding(false); setSelectedType(null); return saved;
  }
  function deletePlan(id: string) { removeFinancialPlan(id); setPlans(loadFinancialPlans()); }

  return <div className="flex flex-col gap-6">
    <div><h1 className="text-2xl font-semibold text-forest-900">Proteksi Finansial</h1><p className="mt-1 text-[14px] text-charcoal/60">Lindungi rencana berharga yang sedang kamu bangun.</p></div>

    <Card className="overflow-hidden bg-gradient-to-br from-white to-forest-50/50">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><div className="mb-2 inline-flex items-center gap-2 rounded-full bg-forest-50 px-3 py-1.5 text-[12px] font-medium text-forest-700"><Shield size={14}/> Protection Planner</div><h2 className="text-[22px] font-semibold text-forest-900">Cari tahu apa yang perlu tetap berjalan.</h2><p className="mt-2 max-w-2xl text-[13px] leading-6 text-charcoal/60">Cashflow dashboard dan hasil kalkulatormu bisa dipakai sebagai dasar. Kamu tidak perlu mengulang data yang sudah pernah dihitung.</p></div><div className="rounded-xl border border-forest-100 bg-white px-5 py-4 lg:min-w-[220px]"><p className="text-[11px] text-charcoal/50">Masih perlu dilindungi</p><p className="mt-1 text-2xl font-semibold text-forest-900">{formatCompact(totalNeed)}</p><p className="mt-1 text-[11px] text-charcoal/45">dari rencana yang kamu simpan</p></div></div>
    </Card>
